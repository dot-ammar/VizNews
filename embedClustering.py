...
def build_embeddings_space(embeddings, a, b):
    if b == None:
        b = a
        
    select_embeddings = embeddings[:a]
    remove_embeddings = embeddings[b:]
    
    cut_center = embeddings[a:b]
    
    space_a = np.arange(len(select_embeddings))

    space_b = np.arange(len(remove_embeddings))
    
    space_center = np.arange(len(cut_center))
    
    space_set = np.array([space_a, space_b, space_center])
