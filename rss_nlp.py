...
custom_stop_words = {"said", "say", "told", "u"}
...
def clean_text(text):
    ...
    cleaned_lemmatized_text = re.sub(r"[^A-Za-z\s]", "", text).lower()
    cleaned_lemmatized_text = cleaned_lemmatized_text.split()
    
    cleaned_lemmatized_text = [
        lemmatizer.lemmatize(word)
        for word in cleaned_lemmatized_text
...
class RSSNLP:
    ...
    def get_articles(self):
        for rss_url in tqdm(self.rss_feeds, desc="RSS Feeds"):
            ...
            print(f"{len(feed.entries)} articles in {rss_url}")
            ...
            print(f"{len(self.articles)} articles in total")
    ...
    def collection_add(self):
        ...
        self.collection = self.client.get_collection(
            COLLECTION_NAME, embedding_function=self.embedding_func
        )

        metadata_cols = self.df.drop(columns=["title"]).to_dict(orient="records")

        print("Length of dataframe is: ", len(self.df))
    ...
    def generate_embedding_plot(self):
        ...
        labels, n_clusters, n_noise = embedClustering.dbscanEMB(
            reduced_embeddings, eps=0.37, min_samples=2
        )

        tsne_df = pd.DataFrame(
            reduced_embeddings, columns=["Component 1", "Component 2"]
        )

        tsne_df["title"] = [title for title in documents]
        tsne_df["link"] = [link for link in links] 
        tsne_df["polarity"] = [round(polarity, 2) for polarity in polarities]
        tsne_df["subjectivity"] = [
            round(subjectivity, 2) for subjectivity in subjectivities
        ]
    ...
    def compute_sentiment(self):
        ...
        self.df[["polarity", "subjectivity", "sentiment"]] = self.df[
            "lemmatized_text"
        ].apply(get_sentiment)
